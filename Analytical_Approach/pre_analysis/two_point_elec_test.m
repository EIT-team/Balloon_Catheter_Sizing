...
M=meshio.read('model7_new.vtu'); % THE MESH IS IN MM
save('model7', 'M')
...
phi_offset = 0; 
nElecRing = 2; % number of electrodes in each ring
for iElec=1:nElecRing
% phi_e = (-1).^(iElec) * pi/2 ; % taking 2 opposite points on the ring
phi_e = pi/3*(iElec-1)/nElecRing + phi_offset; %pi/3 for closer 
...
% % J=log10(abs(J));
% 
% meshio.write('TEST_2pairscloser2_elec_model1_z1.vtu',MDL.nodes,MDL.elems,{J},{'J'});
