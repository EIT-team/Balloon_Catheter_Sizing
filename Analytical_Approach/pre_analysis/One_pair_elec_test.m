...
phi_offset = 0; 
nElecRing = 1; % number of electrodes in each ring
for iElec=1:nElecRing
% phi_e = (-1).^(iElec) * pi/2 ; % taking 2 opposite points on the ring
phi_e = 1*pi*(iElec-1)/nElecRing + phi_offset;
...
Protocol= [1 2, 1 2] % [1 3, 1 3] 1 pair -> analytical approach
...
meshio.write('closer20mm_1pair_elec_model5_z1_1.vtu',MDL.nodes,MDL.elems,{J},{'J'});
