...
d=dir(fil);
name2 = 'a1_slices_point_reconstr_img-k=%3.1f-threshold_i=%3.1f.jpg'; 
j = 0;
...
%         out_img = show_fem(img4,[1,1]);
        ...
        index_z1 = find(z > 0.0124 & z < 0.0126);
        index_z2 = find(z > 0.0024 & z < 0.0026);
        
        final_index_z = [index_z1'; index_z2'];
       
        x_polar = atan(y(final_index_z)./x(final_index_z));
        x_polar_new = myatan(y(final_index_z),x(final_index_z)); % interval 0 and 2pi instead of -pi/2 and pi/2
        radius = sqrt(x(final_index_z).^2 + y(final_index_z).^2);
