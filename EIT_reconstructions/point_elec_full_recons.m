...
folder = '15mm_height/loop3/';
fil = fullfile(folder, 'elliptical*.vtu');
d=dir(fil);
name2 = 'dd_full_point16FR_reconstr_img-k=%3.1f-threshold_i=%3.1f.jpg'; 
j = 0;
...
%     title('Protocols')
    ...
    fine_model=meshio.read('15mm_height/16fr/newfinemodel20mm_16FR.vtu'); 
...
%     title('Fine model circular 20mm diameter', 'Fontsize', 20)
    ...
    coarse_model =meshio.read('15mm_height/16fr/newcoarsemodel20mm_16FR.vtu'); 
...
%     title('Coarse model circular 20mm diameter', 'Fontsize', 20)
    ...
    inv_mdl.hyperparameter.value = 10^(-1.25); % check 0.1 to 0.01 ...
...
%         imwrite(out_img_final, sprintf(name2,k,threshold_i))
...
%         set(gca,'FontSize',18)
        ...
        j=j+1;
        dimensions_sectors_max_matrix(:,j)=dimensions_sectors_max'
        dimensions_sectors_angle_matrix(:,j)=angle_max_rad'
    end
...
fid= fopen('DD-FULL-POINT_RECONSTRUCTION-16FR.html','w');
